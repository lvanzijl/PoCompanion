...
### Issue 1: Migrate to NSwag-Generated API Clients

**Title**: Migrate services to use NSwag-generated API clients instead of direct HttpClient

**Labels**: `architecture`, `technical-debt`, `P0`, `breaking-change`

**Description**:

Services in the Client layer are currently using HttpClient directly, which violates UI_RULES.md Section 9. This makes the codebase harder to maintain and violates architectural guidelines.

**Current State**:
- `PoTool.Client/Services/SettingsService.cs` uses HttpClient directly
- `PoTool.Client/Services/TfsConfigService.cs` uses HttpClient directly
- `PoTool.Client/Services/PullRequestService.cs` uses HttpClient directly
- `PoTool.Client/ApiClient/WorkItemsClient.cs` is manually written instead of generated
- nswag.json configuration exists but client generation is not integrated

**Expected State**:
- All API communication uses NSwag-generated clients
- Client generation integrated into build process
- Dependency injection properly configured for generated clients
- All service tests updated to use generated clients

**Architecture Rule Violated**:
UI_RULES.md Section 9: "MUST use API clients generated via OpenAPI / NSwag. MUST NOT call HttpClient directly from UI components."

**Implementation Steps**:
1. Set up NSwag client generation in build process
   - Add pre-build step to generate clients from Swagger spec
   - Configure nswag.json for proper client namespace and structure
   - Ensure generated file is checked into source control or generated on build

2. Generate API clients for all endpoints
   - Settings API
   - TFS Configuration API
   - Pull Request API
   - Work Items API (replace manual implementation)

3. Refactor services to use generated clients
   - Update SettingsService to use generated SettingsClient
   - Update TfsConfigService to use generated TfsConfigClient
   - Update PullRequestService to use generated PullRequestsClient
   - Update WorkItemService to use fully generated WorkItemsClient

4. Update dependency injection configuration
   - Register generated clients with proper lifetime
   - Configure HttpClient base address
   - Add any necessary delegating handlers

5. Update all service tests
   - Mock generated clients instead of HttpClient
   - Verify all API communication scenarios
   - Ensure error handling works with generated exceptions

6. Verify integration tests still pass
   - Ensure API contracts match generated clients
   - Test error scenarios with new clients

**Acceptance Criteria**:
- [ ] All services use NSwag-generated clients
- [ ] No direct HttpClient usage in Client services
- [ ] Client generation integrated into build
- [ ] All tests passing (unit, integration, Blazor)
- [ ] DI properly configured for generated clients
...
- PoTool.Client/ApiClient/WorkItemsClient.cs
- PoTool.Client/nswag.json
- PoTool.Client/Program.cs (DI configuration)
- All service test files

**References**:
- COMPLIANCE_FINDINGS.md Section 1.1
- UI_RULES.md Section 9
