...
- **Health hub tiles should not automatically be treated as missing signals.**  
  The documented intent in `docs/architecture/navigation-map.md` is that `/home/health` stays lightweight and loads no Health data on entry. Making those tiles dynamic would cut against the current hub contract.
...
- **Health hub should remain lightweight and non-loading on entry.**  
  `docs/architecture/navigation-map.md` explicitly says the Health workspace hub “loads no Health data on entry” and only routes to subpages.
