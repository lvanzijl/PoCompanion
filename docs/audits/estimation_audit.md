...
| --- | --- | --- | --- | --- |
| P0 | `PoTool.Integrations.Tfs/Clients/RealTfsClient.WorkItemsHierarchy.cs` | `RealTfsClient` | `ParseEffortField` | Falls back from `Effort` to `StoryPoints` and returns a single integer estimate. This collapses effort hours and story points into one field, making it impossible for downstream code to apply the domain rule that story points and effort are separate semantics with different rollup rules. |
...
| P0 | `PoTool.Api/Handlers/Metrics/GetSprintMetricsQueryHandler.cs` | `GetSprintMetricsQueryHandler` | `Handle` | `CompletedStoryPoints` and `PlannedStoryPoints` are both computed from `wi.Effort`. This violates multiple domain rules at once: velocity must use story points, not effort hours; only PBIs are authoritative story-point sources; bugs/tasks must not pollute velocity; and missing estimates must not silently become zero through `Effort`-based summation. |
