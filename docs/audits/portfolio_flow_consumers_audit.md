...
| --- | --- | --- | --- | --- | --- |
| `PoTool.Api/Handlers/Metrics/GetPortfolioProgressTrendQueryHandler.cs` | Query handler | Canonical portfolio progress / flow reader | `PortfolioFlowProjectionEntity` | Migrated | Aggregates per-sprint product rows into portfolio totals using `StockStoryPoints`, `RemainingScopeStoryPoints`, `InflowStoryPoints`, and `ThroughputStoryPoints`. |
...
| `PoTool.Api/Handlers/Metrics/GetPortfolioDeliveryQueryHandler.cs` | Query handler | Portfolio delivery distribution snapshot | `SprintMetricsProjectionEntity` | Out of scope for PortfolioFlow | Still aggregates `CompletedPbiEffort`; this is portfolio delivery composition, not the portfolio stock/flow trend consumer. |
| `PoTool.Shared/Metrics/PortfolioDeliveryDtos.cs` | DTO surface | Portfolio delivery transport | Legacy `CompletedEffort` naming over delivery aggregates | Out of scope for PortfolioFlow | Uses legacy property names for compatibility and delivery presentation, not as a PortfolioFlow reader. |
...
- `PoTool.Api/Handlers/Metrics/GetPortfolioDeliveryQueryHandler.cs`, which still aggregates `SprintMetricsProjectionEntity.CompletedPbiEffort` into `PortfolioDeliveryDto.CompletedEffort`
...
3. The handler derives portfolio-level `CompletionPercent` from aggregated stock and remaining scope totals, which is required because the response rolls up multiple product rows into one portfolio data point.
...
- Decide separately whether `GetPortfolioDeliveryQueryHandler.cs` should continue using sprint delivery projections or move to a clearer canonical delivery projection/naming model; this handler is portfolio-adjacent but is not the canonical PortfolioFlow consumer audited here.
