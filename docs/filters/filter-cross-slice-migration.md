...
| --- | --- | --- | --- |
| PR | `GET /api/pullrequests/metrics` → `GetPullRequestMetricsQueryHandler` | Boundary: `PullRequestsController` parses `productIds` CSV.<br>Downstream: handler reads `query.ProductIds` directly and enforces its own 6-month default when `FromDate` is null. | No |
| PR | `GET /api/pullrequests/filter` → `GetFilteredPullRequestsQueryHandler` | Boundary: controller parses `productIds`.<br>Downstream: handler applies `IterationPath`, `CreatedBy`, `ToDate`, and `Status` inline against the returned PR list. | No |
| PR | `GET /api/pullrequests/sprint-trends` → `GetPrSprintTrendsQueryHandler` | Boundary: controller parses `productIds`.<br>Downstream: handler uses `ProductIds` first, otherwise resolves `TeamId` through `ProductTeamLinks`, then maps PRs into sprint windows locally. | No |
| PR | `GET /api/pullrequests/insights` → `GetPullRequestInsightsQueryHandler` | Boundary: controller assigns default `fromDate`/`toDate`.<br>Downstream: handler resolves `TeamId` to linked products, filters by date range and optional `RepositoryName`, then aggregates locally. | No |
| PR | `GET /api/pullrequests/delivery-insights` → `GetPrDeliveryInsightsQueryHandler` | Boundary: controller assigns default dates.<br>Downstream: handler optionally overrides them from `SprintId`, resolves `TeamId` to products and then to repository names, then filters PRs locally. | No |
| Pipeline | `GET /api/pipelines/metrics` → `GetPipelineMetricsQueryHandler` | Boundary: controller parses `productIds` CSV.<br>Downstream: handler loops `query.ProductIds`, resolves pipeline definitions product by product, then hard-codes a 6-month `refs/heads/main` run filter. | No |
| Pipeline | `GET /api/pipelines/runs` → `GetPipelineRunsForProductsQueryHandler` | Boundary: controller parses `productIds` CSV.<br>Downstream: handler repeats the same per-product pipeline-definition resolution and hard-coded 6-month `refs/heads/main` filter. | No |
| Pipeline | `GET /api/pipelines/definitions` → `GetPipelineDefinitionsQueryHandler` | Boundary: query accepts `productId` or `repositoryId`.<br>Downstream: handler branches directly on those request fields and throws if neither is present. | No |
| Pipeline | `GET /api/pipelines/insights` → `GetPipelineInsightsQueryHandler` | Boundary: query uses `productOwnerId`, `sprintId`, and inclusion toggles.<br>Downstream: handler resolves owner products, derives the sprint window, and applies branch filtering per default branch inside the handler/service path. | No |
| Delivery | `GET /api/metrics/portfolio-progress-trend` → `GetPortfolioProgressTrendQueryHandler` | Boundary: query accepts `productOwnerId`, `sprintIds`, optional `productIds`.<br>Downstream: handler loads owner products and intersects requested `productIds` locally before projection lookup. | No |
| Delivery | `GET /api/metrics/capacity-calibration` → `GetCapacityCalibrationQueryHandler` | Boundary: owner + sprint-range + optional product filter contract.<br>Downstream: handler resolves the same scope directly instead of consuming a canonical filter. | No |
| Delivery | `GET /api/metrics/portfolio-delivery` → `GetPortfolioDeliveryQueryHandler` | Boundary: query accepts `productOwnerId` and `sprintIds`.<br>Downstream: handler resolves owner products and sprint range directly, with no shared canonical filter object. | No |
| Delivery | `GET /api/buildquality/rolling` → `GetBuildQualityRollingWindowQueryHandler` + `BuildQualityScopeLoader` | Boundary: query accepts `productOwnerId` and explicit window.<br>Downstream: scope loader resolves products, repository/pipeline constraints, default branches, and time window locally. | No |
| Delivery | `GET /api/buildquality/sprint` → `GetBuildQualitySprintQueryHandler` + `BuildQualityScopeLoader` | Boundary: query accepts `productOwnerId` and `sprintId`.<br>Downstream: handler turns the sprint into a window and delegates all scope resolution to the local build-quality scope loader. | No |
| Pipeline | `GET /api/buildquality/pipeline` → `GetBuildQualityPipelineDetailQueryHandler` + `BuildQualityScopeLoader` | Boundary: query accepts `productOwnerId`, `sprintId`, optional `pipelineDefinitionId`, optional `repositoryId`.<br>Downstream: handler derives the sprint window and loader resolves product/pipeline/repository/default-branch scope locally. | No |
| Sprint | `GET /api/metrics/sprint-trend` → `GetSprintTrendMetricsQueryHandler` | Boundary: query accepts `productOwnerId` and `sprintIds`.<br>Downstream: handler uses request sprint IDs directly for projection selection and current/previous sprint window derivation. | No |
| Sprint | `GET /api/metrics/sprint-execution` → `GetSprintExecutionQueryHandler` | Boundary: query accepts `productOwnerId`, `sprintId`, optional `productId`.<br>Downstream: handler resolves owner products, optionally narrows to one product, and reconstructs scope locally. | No |
| Sprint | `GET /api/metrics/sprint` → `GetSprintMetricsQueryHandler` | Boundary: query accepts `iterationPath`.<br>Downstream: the sprint identifier is interpreted directly in the legacy sprint metrics path instead of through canonical time resolution. | No |
| Sprint | `GET /api/metrics/backlog-health` → `GetBacklogHealthQueryHandler` | Boundary: query accepts `iterationPath`.<br>Downstream: handler/service family uses the raw path as its filter. | No |
| Sprint / Health | `GET /api/metrics/multi-iteration-health` → `GetMultiIterationBacklogHealthQueryHandler` | Boundary: query accepts `productIds`, `areaPath`, and `maxIterations`.<br>Downstream: handler prioritizes product-root loading when `productIds` are present and otherwise falls back to area-path filtering. | No |
| Sprint / Planning | `GET /api/metrics/capacity-plan` → `GetSprintCapacityPlanQueryHandler` | Boundary: query accepts `iterationPath` and optional capacity override.<br>Downstream: iteration selection remains a raw request field. | No |
| Sprint | `GET /api/metrics/work-item-activity/{workItemId}` → `GetWorkItemActivityDetailsQueryHandler` | Boundary: query accepts explicit period bounds.<br>Downstream: the handler applies them directly in the query path. | No |
| Delivery / Home | `GET /api/metrics/home-product-bar` → `GetHomeProductBarMetricsQueryHandler` | Boundary: query accepts `productOwnerId` and optional `productId`.<br>Downstream: handler resolves owner scope locally, but only partially applies `productId` across the returned metrics. | No |
| Health | `GET /api/workitems/validated` → `GetAllWorkItemsWithValidationQueryHandler` | Boundary: controller parses `productIds` CSV.<br>Downstream: handler branches between selected products, all products, or profile-area fallback and then validates the loaded set. | No |
| Health | `GET /api/workitems/validated/{tfsId}` → `GetWorkItemByIdWithValidationQueryHandler` | Boundary: controller parses `productIds`.<br>Downstream: handler does not enforce them; the accepted parameter is effectively ignored today, which should be treated as a current correctness bug as well as a migration gap. | No |
| Health | `GET /api/workitems/validation-triage` → `GetValidationTriageSummaryQueryHandler` | Boundary: controller parses `productIds`.<br>Downstream: handler passes them to `GetAllWorkItemsWithValidationQuery` and builds category aggregates locally. | No |
| Health | `GET /api/workitems/validation-queue` → `GetValidationQueueQueryHandler` | Boundary: controller parses `productIds`.<br>Downstream: handler combines the raw category key with locally loaded product-scoped work items. | No |
| Health | `GET /api/workitems/validation-fix` → `GetValidationFixSessionQueryHandler` | Boundary: controller parses `productIds`.<br>Downstream: handler combines raw `ruleId`/`category` with locally filtered work items. | No |
| Health | `GET /api/workitems/validation-impact-analysis` → `GetValidationImpactAnalysisQueryHandler` | Boundary: query accepts `areaPathFilter` and `iterationPathFilter`.<br>Downstream: handler loads product-scoped work items first and then applies both filters inline. | No |
| Health | `GET /api/workitems/advanced-filter` → `GetFilteredWorkItemsAdvancedQueryHandler` | Boundary: query exposes type, state, iteration, area, effort, blocked, title, and validation flags.<br>Downstream: handler applies every filter directly over the loaded work-item set. | No |
| Health | `GET /api/workitems/backlog-state/{productId}` → `GetProductBacklogStateQueryHandler` | Boundary: route carries `productId`.<br>Downstream: handler loads the product backlog graph directly and resolves scope locally. | No |
| Health | `GET /api/workitems/health-summary/{productId}` → `GetHealthWorkspaceProductSummaryQueryHandler` | Boundary: route carries `productId`.<br>Downstream: handler loads one product backlog tree and computes summary metrics locally. | No |
...
| `PoTool.Api/Handlers/Metrics/GetHomeProductBarMetricsQueryHandler.cs:41-53` vs `119-164` | `productId` only narrows bug and change counts. Sprint progress is still computed from all owner-linked teams/products. This is a current correctness bug, not only a migration smell. | A single response mixes filtered and unfiltered semantics, which can already mislead users today and will also break UI assumptions once global product scope becomes canonical elsewhere. |
...
- All five areas are marked `High` severity in section 5 because each can produce incorrect analytical results, but the migration order is based on coupling and comparison difficulty rather than severity alone.

## 5. Risk Analysis

| Area | Risk | Severity | Mitigation |
| --- | --- | --- | --- |
| Delivery | **Incorrect filtering:** owner/product scope is resolved separately in portfolio progress, capacity calibration, portfolio delivery, home product bar, and build-quality flows.<br>**Performance:** dual-path comparison may temporarily duplicate projection or aggregate work.<br>**Snapshot/history:** delivery projections, build-quality facts, and home-product rollups can disagree if product scope is normalized differently.<br>**UI breakage:** cards that currently assume owner-wide data, especially home product bar, may change visibly when product scope becomes consistent. | High | Migrate delivery endpoints as a coordinated family; compare product IDs and aggregate totals before cutover; explicitly separate canonical scope from page-local display defaults. |
| Sprint | **Incorrect filtering:** sprint selection is represented as `iterationPath`, `sprintId`, or `sprintIds` depending on the endpoint.<br>**Performance:** resolving sprint windows centrally may add extra lookup work during dual path.<br>**Snapshot/history:** some paths are projection-based while others reconstruct history from activity events, so a changed time window can shift both counts and chronology.<br>**UI breakage:** sprint pages may lose or gain items if their legacy iteration matching does not align with the canonical time model. | High | Standardize sprint/time mapping at the boundary first, then compare selected sprint IDs, window boundaries, and work-item counts per page before switching each endpoint. |
| Pipeline | **Incorrect filtering:** pipeline metrics and runs use hard-coded `refs/heads/main`, while pipeline insights and build quality use default-branch-aware filtering.<br>**Performance:** central resolution may widen or narrow candidate definition sets, affecting query volume.<br>**Snapshot/history:** the same sprint or time range may include different run populations across endpoints.<br>**UI breakage:** top-trouble pipelines and failure rates can move noticeably once branch scope is unified. | High | Treat branch/default-branch selection as an explicit slice-local option; compare selected run IDs and definition IDs during dual path; migrate pipeline insights and build-quality scope together. |
| PR | **Incorrect filtering:** `TeamId` and date-range semantics differ across metrics, insights, sprint trends, and delivery insights.<br>**Performance:** central resolution may add one shared scope lookup but should reduce repeated handler work over time.<br>**Snapshot/history:** sprint-derived PR windows and free date ranges can classify the same PR differently today.<br>**UI breakage:** PR overview and delivery insights may show changed totals or repository breakdowns when team/product scope is normalized. | High | Start with boundary comparison logging for team/product/date resolution; verify PR ID sets, repository buckets, and per-sprint counts before enabling the effective path. |
| Health | **Incorrect filtering:** product-root loading, profile-area fallback, area-path filters, and validation drill-downs all define scope locally.<br>**Performance:** dual-path work can be expensive because some handlers currently load broad work-item sets and then filter in memory.<br>**Snapshot/history:** health and validation endpoints can analyze different work-item universes for the same product selection.<br>**UI breakage:** triage counts, validation queues, backlog-state trees, and health summary cards may change once hidden fallback behavior is removed. | High | Migrate health last; first document and compare work-item ID sets per endpoint; enforce product/profile defaults at the boundary; add regression tests around validation queue counts and health summary totals before switch-over. |
