...
Existing documentation already flagged several of these risks, especially sprint scoped loading, build-quality branch filtering, and pipeline insights branch filtering (`docs/filters/filter-validation-report.md:255-319`). The remaining workstream should therefore focus on:
...
- sprint scoped work-item loading N+1 + in-memory filtering (`docs/filters/filter-validation-report.md:257-273`)
- build-quality branch filtering after materialization (`docs/filters/filter-validation-report.md:275-289`)
- pipeline insights branch filtering after materialization (`docs/filters/filter-validation-report.md:291-304`)
...
The repository rules already require UTC `DateTime` columns for queryable timestamps and explicitly forbid server-side `DateTimeOffset` predicates/sorts on SQLite (`docs/EF_RULES.md:130-149`). The filtering paths generally follow that rule:
...
- sprint/domain recomputation that needs first-done, commitment, spillover, and hierarchy propagation to match canonical rules (`docs/domain/rules/sprint_rules.md:27-97`, `docs/domain/rules/propagation_rules.md:46-90`, `docs/domain/rules/metrics_rules.md:21-66`)
