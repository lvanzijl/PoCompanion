...
**Purpose:** Workspace-first Product Owner Dashboard. Shows four dominant workspace tiles as the primary navigation, a horizontal product context bar, lightweight quick actions, and a compact sync footer.

| Functionality | Description |
|---|---|
| Workspace tiles | Shows exactly four standardized, clickable filled-card tiles in a 2×2 grid: Health, Delivery, Trends, and Planning. Each tile uses a raised card surface (elevation 2) with a two-column horizontal layout: workspace icon anchored in a fixed-width left column, and title (bold) above signal text in the right column. The tiles are intentionally visually dominant so the Home page reads first as workspace navigation. The tile shells render immediately, then each tile loads its own signal independently with a compact progress indicator below the title so one failing signal does not block the other tiles. |
| Product bar | Shows product selector chips on the left side of a single compact horizontal bar — no label prefix — with horizontal chip scrolling when needed. Selecting a chip highlights the active context and propagates that context to supported workspace links. The bar shell renders immediately and fills progressively with product chips plus three compact muted contextual metrics on one line: a team-wide sprint progress percentage plus bug count and changes-today count for the active product scope. |
...
| Sync footer | A compact, right-aligned line at the bottom of the page shows the last sync time and a manual Sync action. The footer renders immediately and replaces its placeholder with the latest sync state when available. |

**Outgoing navigation:** `/home/health`, `/home/delivery`, `/home/trends`, `/home/planning`, `/home/validation-triage`, `/bugs-triage`, `/profiles` (if no profile)
...
| Functionality | Description |
|---|---|
| Breadcrumb | `Home › Health › Overview`. |
...
```
Sprint Delivery
  → Portfolio summary (aggregated delivery per product)
      → Product (epic delivery breakdown)
          → Epic (feature delivery breakdown)
              → Feature (completed PBIs)
```

Each level only appears when the user drills into the previous one. Users navigate forward by clicking a row and backward via the drill-down breadcrumb bar.
...
**Current situation:** The current navigation (`/home` and its workspaces) is now the sole visible navigation model. The legacy `/legacy` entry point has been removed, while the residual `/workspace/*` routes remain only as direct-entry legacy pages pending later cleanup.

**Suggestion:** Complete the staged cleanup by removing the remaining `/workspace/*` legacy routes once their embedded or modern replacements are explicitly confirmed.
