...
| Functionality | Description |
|---|---|
| Workspace tiles | Shows exactly four standardized, clickable filled-card tiles in a 2×2 grid: Health, Delivery, Trends, and Planning. Each tile uses a raised card surface (elevation 2) with a two-column horizontal layout: workspace icon anchored in a fixed-width left column, and title (bold) above signal text in the right column. The tiles are intentionally visually dominant so the Home page reads first as workspace navigation. Each tile shows exactly one short signal line: the highest-priority workspace signal in the active product context, or a neutral confirmation signal when no negative signal exists. Tiles lift slightly on hover for clear interactive feedback. |
| Product bar | Shows product selector chips on the left side of a single compact horizontal bar — no label prefix — with horizontal chip scrolling when needed. Selecting a chip highlights the active context and propagates that context to supported workspace links. The right side shows exactly three compact muted contextual metrics on one line: a team-wide sprint progress percentage plus bug count and changes-today count for the active product scope. |
