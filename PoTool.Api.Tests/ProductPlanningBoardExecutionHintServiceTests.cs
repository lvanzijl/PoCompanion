...
public sealed class ProductPlanningBoardExecutionHintServiceTests
{
    ...
    public async Task ApplyExecutionHintAsync_PrefersStrongAnomalyOverWeakAndMapsExactText()
    {
        ...
        Assert.AreEqual("Execution signal: committed work was not delivered as expected (recent sprint)", result.ExecutionHint.Message);
        Assert.AreEqual("Recent sprint delivered less than committed. Open Sprint Execution to see unfinished work.", result.ExecutionHint.Explanation);
    ...
    public async Task ApplyExecutionHintAsync_UsesPhase26TieBreakOrderForEqualSeverity()
    {
        ...
        Assert.AreEqual("Execution signal: committed work kept carrying into the next sprint (recent sprint)", result.ExecutionHint.Message);
    }
