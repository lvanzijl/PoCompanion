...
public sealed class StateClassificationsAnalysisDocumentTests
{
    ...
    public void StateClassificationsAnalysis_ReportExistsWithRequiredSections()
    {
        ...
        var reportPath = Path.Combine(repositoryRoot, "docs", "analyze", "state-classifications.md");

        Assert.IsTrue(File.Exists(reportPath), "The state classifications analysis report should exist under docs/analyze.");
