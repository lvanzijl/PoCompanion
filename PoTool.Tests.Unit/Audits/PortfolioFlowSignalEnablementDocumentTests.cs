...
public sealed class PortfolioFlowSignalEnablementDocumentTests
{
    ...
    public void PortfolioFlowSignalEnablement_ReportExistsWithRequiredSections()
    {
        ...
        var reportPath = Path.Combine(repositoryRoot, "docs", "audits", "portfolio_flow_signal_enablement.md");

        Assert.IsTrue(File.Exists(reportPath), "The portfolio flow signal enablement audit should exist under docs/audits.");
