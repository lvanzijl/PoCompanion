...
public sealed class MetricsControllerDeliveryCanonicalFilterTests
{
    ...
    [TestMethod]
    public async Task GetPortfolioDelivery_WrapsResponseWithCanonicalFilterMetadata()
    {
        await using var context = CreateContext();
        PersistenceTestGraph.EnsureProject(context);
        PersistenceTestGraph.EnsureTeam(context, 3);
        ...
        mediator
            .Setup(service => service.Send(
                It.Is<GetPortfolioDeliveryQuery>(query =>
                    query.EffectiveFilter.Context.ProductIds.Values.SequenceEqual(new[] { 100 })
                    && query.EffectiveFilter.SprintIds.SequenceEqual(new[] { 42 })),
                It.IsAny<CancellationToken>()))
        ...
        var result = await controller.GetPortfolioDelivery(7, [42], [100], CancellationToken.None);

        var ok = result.Result as OkObjectResult;
        Assert.IsNotNull(ok);

        var envelope = ok.Value as DeliveryQueryResponseDto<PortfolioDeliveryDto>;
        Assert.IsNotNull(envelope);
